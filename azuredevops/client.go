...
func CreatePullRequestComment(azureDevOpsParameters *AzureDevOpsParameters, content string) {
	...
	if err != nil {
		utils.Logger.Fatal("Creating HTTP client for Azure DevOps 'git' section failed.", zap.Error(err))
	}
	...
	if err != nil {
		utils.Logger.Fatal("Creating new comment thread on pull request failed.", zap.Error(err))
	}
